...
export function CategoryLanding({ slug }: { slug: CategorySlug }) {
  ...
  const structuredData = [
    ...
    { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: category.faq.map((item) => ({ "@type": "Question", name: item.q, acceptedAnswer: { "@type": "Answer", text: item.a } })) },
  ];
  return <main>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} />
    <nav className="nav"><a className="brand" href="/"><span>Calcul</span><b>Argent</b></a><div className="navLinks"><a href="/epargne">Épargne</a><a href="/dettes">Dettes</a><a href="/patrimoine">Patrimoine</a><a href="/budget">Budget</a><a href="/guides">Guides</a><a href="/lexique">Lexique</a></div><a className="back" href="/#outils">Tous les outils</a></nav>
    ...
    <section className="categoryEditorial"><div className="categoryInsightGrid">{category.insights.map((item) => <article key={item.title}><h2>{item.title}</h2><p>{item.body}</p></article>)}</div><div className="categoryFaq"><span className="kicker dark">QUESTIONS FRÉQUENTES</span>{category.faq.map((item) => <details key={item.q}><summary>{item.q}<b>+</b></summary><p>{item.a}</p></details>)}</div></section>
